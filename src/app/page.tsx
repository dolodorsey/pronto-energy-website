  ...
  return (
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: phase < 2 ? 10000 : -1, background: C.dark, display: "flex", alignItems: "center", justifyContent: "center", opacity: phase >= 2 ? 0 : 1, transition: "opacity 0.8s cubic-bezier(0.16,1,0.3,1)", pointerEvents: phase >= 2 ? "none" : "all" }}>
      <div style={{ width: "100vw", height: "100vh", overflow: "hidden", position: "relative" }}>
        <video autoPlay muted loop playsInline style={{ width: "100%", height: "100%", objectFit: "cover", filter: "brightness(0.5) contrast(1.2) saturate(0.7)" }}>
    ...
    >
      <video autoPlay muted loop playsInline style={{ position: "absolute", inset: "-5%", width: "110%", height: "110%", objectFit: "cover", filter: "brightness(0.18) contrast(1.3) saturate(0.5)", transform: `scale(1.02) translate(${(mouse.x - 0.5) * -8}px,${(mouse.y - 0.5) * -8}px)`, transition: "transform 0.3s ease" }}>
        <source src="/videos/portal.mp4" type="video/mp4" />
      </video>
      ...
      <div style={{ position: "relative", zIndex: 2, padding: "160px clamp(32px,8vw,120px) 120px", maxWidth: 1400, margin: "0 auto", width: "100%" }}>
        <div style={{ opacity: phase >= 2 ? 1 : 0, transform: phase >= 2 ? "translateY(0)" : "translateY(60px)", transition: "all 1.4s cubic-bezier(0.16,1,0.3,1) 0.2s" }}>
          <div style={{ fontFamily: "'DM Sans',system-ui", fontSize: 9, fontWeight: 600, letterSpacing: "0.7em", textTransform: "uppercase", color: C.volt, marginBottom: 32, display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ marginTop: "clamp(28px,4vw,52px)", marginLeft: "clamp(0px,6vw,100px)", maxWidth: 500, opacity: phase >= 2 ? 1 : 0, transition: "opacity 1s ease 1s" }}>
            <p style={{ fontFamily: "'DM Sans',system-ui", fontSize: "clamp(15px,1.3vw,19px)", fontWeight: 300, lineHeight: 1.85, color: C.silver, opacity: 0.65 }}>Five bold flavors. Zero sugar. Clean energy for the ones who move first — from the gym floor to the festival ground.</p>
            <div style={{ display: "flex", gap: 14, marginTop: 40, flexWrap: "wrap" }}>
              <button style={{ fontFamily: "'DM Sans',system-ui", fontSize: 11, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: C.dark, background: C.volt, border: "none", padding: "16px 48px", cursor: "pointer", transition: "all 0.4s cubic-bezier(0.16,1,0.3,1)" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.boxShadow = `0 16px 48px ${C.volt}40` }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "none" }}>Shop Now</button>
              <button style={{ fontFamily: "'DM Sans',system-ui", fontSize: 11, fontWeight: 400, letterSpacing: "0.16em", textTransform: "uppercase", color: C.white, background: "transparent", border: `1px solid ${C.dim}`, padding: "16px 40px", cursor: "pointer", transition: "all 0.3s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = C.glacier; e.currentTarget.style.color = C.glacier }} onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)"; e.currentTarget.style.color = C.white }}>Wholesale</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {[
        ...
        <div key={i} style={{ position: "relative", aspectRatio: "16/9", overflow: "hidden", cursor: "pointer" }}
          ...
          <div style={{ position: "absolute", bottom: 32, left: 36 }}>
            <div style={{ fontFamily: "'DM Sans',system-ui", fontSize: 8, fontWeight: 700, letterSpacing: "0.5em", textTransform: "uppercase", color: s.accent }}>{s.label}</div>
...
function ElementalHeroes() {
  ...
  return (
    <section style={{ background: C.dark, padding: "120px 0 80px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ padding: "0 clamp(32px,8vw,120px)", maxWidth: 1400, margin: "0 auto 56px" }}>
        <Rev><div style={{ fontFamily: "'DM Sans',system-ui", fontSize: 9, fontWeight: 700, letterSpacing: "0.6em", textTransform: "uppercase", color: C.volt, marginBottom: 20 }}>Elemental Energy</div>
      ...
      <div className="elemental-grid" style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 2, padding: "0 2px" }}>
        {heroes.map((h, i) => (
  ...
  return (
    <section style={{ position: "relative", height: "60vh", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", bottom: "clamp(28px,5vh,60px)", left: "clamp(32px,8vw,120px)", zIndex: 2 }}>
        <Rev><div style={{ fontFamily: "'DM Sans',system-ui", fontSize: 8, fontWeight: 700, letterSpacing: "0.5em", textTransform: "uppercase", color: C.volt }}>The Pronto Life</div>
    ...
    <section id="flavors" style={{ background: C.base, padding: "120px clamp(32px,8vw,120px)" }}>
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        <Rev><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 24, marginBottom: 60 }}>
          <div>
            <div style={{ fontFamily: "'DM Sans',system-ui", fontSize: 9, fontWeight: 700, letterSpacing: "0.6em", textTransform: "uppercase", color: C.volt, marginBottom: 20 }}>6 Bold Flavors</div>
            ...
            <Rev key={p.name} d={0.04 * i}>
              <div onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(null)} style={{ background: p.bg, padding: "24px 16px 28px", cursor: "pointer", transition: "all 0.4s cubic-bezier(0.16,1,0.3,1)", position: "relative", overflow: "hidden", borderTop: `2px solid ${hover === i ? p.accent : "transparent"}` }}>
                ...
                <div style={{ position: "relative", zIndex: 1 }}>
                  ...
                  <p style={{ fontFamily: "'DM Sans',system-ui", fontSize: 11, fontWeight: 300, lineHeight: 1.65, color: C.muted }}>{p.desc}</p>
                </div>
              </div>
            </Rev>
          ))}
    ...
    <section id="science" style={{ background: C.dark, padding: "120px clamp(32px,8vw,120px)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <Rev><div className="science-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
          <div>
            <div style={{ fontFamily: "'DM Sans',system-ui", fontSize: 9, fontWeight: 700, letterSpacing: "0.6em", textTransform: "uppercase", color: C.volt, marginBottom: 20 }}>The Formula</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
              {facts.map(f => (<div key={f.label} style={{ background: C.surface, padding: "22px 18px" }}>
                ...
                <div style={{ fontFamily: "'DM Sans',system-ui", fontSize: 13, fontWeight: 600, color: C.white, lineHeight: 1.4 }}>{f.value}</div>
              </div>))}
            </div>
          </div>
      ...
      {[
        ...
        <Rev key={i} d={0.05 * i}>
          <div style={{ position: "relative", aspectRatio: "3/4", overflow: "hidden", cursor: "pointer" }}
            ...
            <div style={{ position: "absolute", bottom: 20, left: 20, fontFamily: "'DM Sans',system-ui", fontSize: 8, fontWeight: 700, letterSpacing: "0.45em", textTransform: "uppercase", color: C.volt }}>{s.label}</div>
          </div>
        </Rev>
      ))}
    </section>
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        ...
        <div className="retail-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 2 }}>
          {channels.map((ch, i) => (<Rev key={ch.title} d={0.06 * i}><div style={{ background: C.dark, padding: "48px 40px", position: "relative", overflow: "hidden", borderLeft: `2px solid ${i % 2 === 0 ? C.volt : C.glacier}` }}>
            <div style={{ fontFamily: "'DM Sans',system-ui", fontSize: 9, fontWeight: 700, letterSpacing: "0.45em", textTransform: "uppercase", color: C.volt, marginBottom: 16 }}>0{i + 1}</div>
  ...
  return (
    <section style={{ background: C.dark, padding: "160px clamp(32px,8vw,120px)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center", position: "relative", zIndex: 2 }}>
        <Rev>
          <div style={{ fontFamily: "'DM Sans',system-ui", fontSize: 9, fontWeight: 700, letterSpacing: "0.7em", textTransform: "uppercase", color: C.volt, marginBottom: 28 }}>No Crash. No Excuse.</div>
          ...
          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
            <button style={{ fontFamily: "'DM Sans',system-ui", fontSize: 11, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: C.dark, background: C.volt, border: "none", padding: "16px 56px", cursor: "pointer", transition: "all 0.4s" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.boxShadow = `0 16px 48px ${C.volt}40` }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "none" }}>Shop Pronto</button>
            <button style={{ fontFamily: "'DM Sans',system-ui", fontSize: 11, fontWeight: 400, letterSpacing: "0.16em", textTransform: "uppercase", color: C.white, background: "transparent", border: `1px solid ${C.dim}`, padding: "16px 44px", cursor: "pointer" }}>Wholesale</button>
          </div>
        </Rev>
      </div>
    </section>
  ...
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 9999, padding: s ? "10px clamp(24px,6vw,80px)" : "24px clamp(24px,6vw,80px)", display: "flex", justifyContent: "space-between", alignItems: "center", background: s ? `${C.base}f0` : "transparent", backdropFilter: s ? "blur(32px) saturate(1.3)" : "none", borderBottom: s ? `1px solid ${C.border}` : "none", transition: "all 0.5s cubic-bezier(0.16,1,0.3,1)" }}>
      ...
      <div style={{ display: "flex", gap: "clamp(14px,2.5vw,36px)", alignItems: "center" }}>
        ...
        <button style={{ fontFamily: "'DM Sans',system-ui", fontSize: 9, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: C.dark, background: C.volt, border: "none", padding: "9px 24px", cursor: "pointer" }}>Get Pronto</button>
      </div>
    </nav>
  );
}
